...
data0.enumIndex = 0;
var data1 = rom.myCreateParameter(rom.DoubleParameter, rom.Input, as, 'F', 10, '端点集中力', 'N');
...
data0.respondChangedFunction = function() {
    ...
    var index =sf.parameters.length-1;
    while(index>0){
        sf.remove(sf.parameters[index]);
        index--;
    }
    ...
    if (self.value == 1) {
        var sf = self.owningSimfun;
        sf.name = '矩形梁';
        var data1 = rom.myCreateParameter(rom.DoubleParameter, rom.Input, sf, 'F', 10, '端点集中力', 'N');
        var data2 = rom.myCreateParameter(rom.DoubleParameter, rom.Input, sf, 'E', 210000, '弹性模量', 'MPa');
        var data3 = rom.myCreateParameter(rom.DoubleParameter, rom.Input, sf, 'L', 100, '梁的长度', 'mm');
