...
r1.scriptFunction = function() {
    ...
    self.parentSimfun.data1.appendRowAt(0, self.time.value);
    self.parentSimfun.data1.appendRowAt(1, self.pressure1.value);
}
...
r2.scriptFunction = function() {
    self.previous.time.value = self.previous.time + 0.1;
    log("time is:" + self.time);
    self.pressure2.value = Math.pow(self.time - 1, 0.4) - 0.4 * self.time;
    log("pressure2 is:" + self.pressure2);
    self.parentSimfun.data1.appendRowAt(0, self.time.value);
    self.parentSimfun.data1.appendRowAt(1, self.pressure2.value);
}
...
r3.scriptFunction = function() {
    self.previous.previous.time.value = self.previous.previous.time + 0.1;
    log("time is:" + self.time);
    self.pressure3.value = Math.pow(self.time - 2, 0.6) - 0.33 * self.time;
    log("pressure3 is:" + self.pressure3);
    self.parentSimfun.data1.appendRowAt(0, self.time);
    self.parentSimfun.data1.appendRowAt(1, self.pressure3);
}
...
r4.scriptFunction = function() {
    self.previous.previous.previous.time.value = self.previous.previous.previous.time + 0.1;
    log("time is:" + self.time);
    self.pressure4.value = Math.pow(self.time - 5, 0.8) - 0.2 * self.time;
    log("pressure4 is:" + self.pressure4);
    self.parentSimfun.data1.appendRowAt(0, self.time);
    self.parentSimfun.data1.appendRowAt(1, self.pressure4);
}
...
r0.setRunConditionFunction(1, function() {
    self.sr2.previous.time.value >= 1 && self.sr2.pressure2.value >= 0 && self.sr2.pressure2.value <= self.sr1.pressure1.value;
});
r0.setRunConditionFunction(2, function() {
    self.sr3.previous.previous.time.value >= 1.3 && self.sr3.pressure3.value >= 0;
});
...
r0.beforeOneIterationFunction = function() {
    log('before: 当前循环次数');
};
r0.afterOneIterationFunction = function() {
    log('after: 当前循环次数');
};
