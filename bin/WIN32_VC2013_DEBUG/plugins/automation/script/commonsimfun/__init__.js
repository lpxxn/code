...
function setNumericProperties(parameter, vobj) {
    var prop = parameter.properties;
    vobj.lowerEnabled = prop.lowerEnabled;
    if (vobj.lowerEnabled) {
        vobj.lowerLimit = prop.lower;
        vobj.greaterEqual = prop.greaterEqual;
    }
    vobj.upperEnabled = prop.upperEnabled;
    if (vobj.upperEnabled) {
        vobj.upperLimit = prop.upper;
        vobj.lessEqual = prop.lessEqual;
    }
    vobj.units = prop.units;
}

function setEnumProperties(parameter, parObj) {
    var enumProp = parameter.enumValue;
    if (enumProp && enumProp.enabled) {
        parObj.enumEnabled = enumProp.enabled;
        parObj.enumValues = enumProp.values;
        parObj.enumNames = enumProp.names;
        parObj.enumIndex = enumProp.index;
    }
}

function setNumberCustomFormat(parameter, parObj) {
    if (parameter.format) {
        parObj.customFormatEnabled = parameter.format.enabled;
        parObj.base = parameter.format.base;
    }
}
...
function createIntegerParameter(name, parameter, sfOrGroup, parObj) {
    ...
    if ('properties' in parameter) {
        setNumericProperties(parameter, parObj);
    }
    setEnumProperties(parameter, parObj);
    setNumberCustomFormat(parameter, parObj);
...
function createIntegerArrayParameter(name, parameter, sfOrGroup, parObj) {
    ...
    if ('properties' in parameter) {
        setNumericProperties(parameter, iv);
    }
    return iv;    
}
    ...
    if ('children' in parameter) {
        ...
        for (var par in parameters) {
            ...
            if (parObj.containParameter(par)) {
                parObj2 = parObj.parameterFromName(par);
            }
...
function createTableParameter(name, parameter, sfOrGroup, parObj) {
    var table = new rom.TableParameter(name, parameter['type'], sfOrGroup);
    if (parameter.columns) {
        for (var i = 0; i < parameter.columns.length; ++i) {
            var column = parameter.columns[i];
            table.appendColumn(column.type, column.size);
            switch (column.type) {
            case rom.TableParameter.Integer:
                table.fillIntegerColumnAt(i, column.value);
                break;
            case rom.TableParameter.Float:
                table.fillFloatColumnAt(i, column.value);
                break;
            case rom.TableParameter.Double:
                table.fillDoubleColumnAt(i, column.value);
                break;
            case rom.TableParameter.IntegerComplex:
                table.fillIntegerComplexColumnAt(i, column.value);
                break;
            case rom.TableParameter.FloatComplex:
                table.fillFloatComplexColumnAt(i, column.value);
                break;
            case rom.TableParameter.DoubleComplex:
                table.fillDoubleComplexColumnAt(i, column.value);
                break;
            case rom.TableParameter.String:
                table.fillStringColumnAt(i, column.value);
                break;
            case rom.TableParameter.DateTime:
                for (var j = 0; j < column.value.length; ++j)
                    column.value[j] = new Date(column.value[j]);
                table.fillDateTimeColumnAt(i, column.value);
                break;
            }
            table.setColumnNameAt(i, column.name);
            table.setColumnLongNameAt(i, column.longName);
            table.setColumnUnitAt(i, column.unit);
            table.setColumnCommentAt(i, column.comment);
            table.setColumnExpressionAt(i, column.expression);
            table.setColumnSignificanceAt(i, column.significance);
        }
    }
...
//                 { menuItem : 'seperator' },
                 { menuItem : qsTranslate('AutoScript', 'Iterative Data Browsing'), actor : 'IterDataBrowsing'},
...
rom.library.parameterGroups = function() {
    var groups=[];
    for (var parameter in rom.library.parameters) {
        var varObj = rom.library.parameters[parameter];
        if (!groups.in_array(varObj.group))
            groups.push(varObj.group);
    }
    return groups;
}

rom.library.parametersInGroup = function(group) {
    var vars=[];
    for (var parameter in rom.library.parameters) {
        var varObj = rom.library.parameters[parameter];
        // 在收集变量数据时，会丢失它本身的标识，因此这里再增加一个id属性
        if (varObj.group == group) {
            varObj['id'] = parameter;
            vars.push(varObj);
        }
    }
...
rom.library.parameterMetaProperty = function(id, property) {
    for (var parameter in rom.library.parameters) {
        if (parameter == id) {
            var varObj = rom.library.parameters[parameter];
            if (property in varObj)
                return varObj[property];            
        }
    }
    return null;
}

rom.library.simfunGroups = function() {
    var groups=[];
    for(var sf in rom.library.simfuns) {
        var sfObj = rom.library.simfuns[sf];
        if (!groups.in_array(sfObj.group))
            groups.push(sfObj.group);
    }
    return groups;
}

rom.library.simfunsInGroup = function(group) {
    var simfuns=[];
    for(var sf in rom.library.simfuns) {
        var sfObj = rom.library.simfuns[sf];
        // 在收集仿函数据时，会丢失它本身的标识，因此这里再增加一个id属性
        if (sfObj.group == group) {
            sfObj['id'] = sf;
            simfuns.push(sfObj);
        }
    }
