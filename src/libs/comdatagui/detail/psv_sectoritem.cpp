...
PSV_SectorItem::PSV_SectorItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param,
                               QGraphicsItem *parent)
    : PSV_Item(param,parent)
    , m_height(30)
    , m_startAngle(270)
    , m_angle(40)
    , m_displacementRatio(0.1)
...
void PSV_SectorItem::setDisplacementRatio(float ratio)
{
...
void PSV_SectorItem::updateItem()
{
    ...
    m_dx = m_displacementRatio * rect_up.width() * 0.5 * cos((m_startAngle + 0.5 * m_angle)* PSV_PI / 180);
    m_dy = -m_displacementRatio * rect_up.height() * 0.5 * sin((m_startAngle + 0.5 * m_angle)* PSV_PI / 180);
